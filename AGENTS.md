...
- 2026-09-19 (Arte 0-5 + QA): `MaterialFactory` único creador de materiales; spawns con `SamplePosition`+`Warp`; `UiLayout.FixSceneUI` estándar UI; `ParqueCaldasDetalle` (catedral, torre, calles, palomas, todo primitivas); balas dañan por `IsHostile`; M3 con derrota; HUD creado en MissionCanvas; `retreatThreshold` 8→12. Detalle en `docs/ESTADO.md` (PASADA QA, MEJORAS, VERIFICACIÓN). Sin compilar: pendiente licencia Unity (hay `.alf` en `Unity-descargas/`) y `git lfs pull` para `.fbx`/texturas reales.
- 2026-09-19 (Repo): rama de trabajo = `master` en `git@github.com:Andres777233/SenaUnity.git` (commit "Arreglo bugs" integra historial previo). Unity 6000.6.2f1 instalado en `/home/andres/Unity/Hub/Editor/6000.6.2f1`; en CachyOS/Arch instalar `libxml2-legacy` por pacman (sin eso, `LD_LIBRARY_PATH` a carpeta con symlink `libxml2.so.2 -> libxml2.so.16`).
