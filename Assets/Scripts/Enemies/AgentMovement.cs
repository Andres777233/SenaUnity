...
namespace Popayork.Enemies
{
    ...
    public class AgentMovement : MonoBehaviour
    {
        ...
        // Coloca al agente sobre el NavMesh (SamplePosition + Warp).
        // Pre-coloca el transform ANTES de activar el GameObject para que Unity
        // no registre "Failed to create agent because it is not close enough
        // to the NavMesh". Devuelve false si no hay punto válido (no activar).
        public bool Place(Vector3 position)
        {
            ...
            cornerIndex = 0;
            Vector3 ground;
            if (!SampleGround(position, out ground))
            {
                return false;
            }
        ...
        // Punto transitable más cercano (radio 4 m). Sin asignaciones.
        public bool SampleGround(Vector3 position, out Vector3 ground)
        {
            NavMeshHit hit;
