...
namespace Popayork.Enemies
{
    public class AgentPool : MonoBehaviour
    {
        ...
        // Si el punto no está sobre el NavMesh, el agente no aparece (warning con el punto).
        public AgentBrain Spawn(Faction faction, Vector3 position, Vector3 objective, string spotName = "?")
        {
            return SpawnFiltered(faction, string.Empty, position, objective, spotName);
        }

        public AgentBrain SpawnFiltered(Faction faction, string variant, Vector3 position, Vector3 objective, string spotName = "?")
        {
            for (int i = 0; i < slotCount; i++)
            {
                ...
                {
                    if (brain.Activate(position, objective, spotName))
                    {
