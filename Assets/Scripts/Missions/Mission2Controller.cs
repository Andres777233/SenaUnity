...
namespace Popayork.Missions
{
    ...
    public class Mission2Controller : MonoBehaviour
    {
        ...
        public void StartDefense()
        {
            ...
            SpawnDefenseAllies();
            if (ActiveWaves() != null && config.defenseWaves != null && config.defenseWaves.Length > 0)
            {
        ...
        private void SimulateDefense(float dt)
        {
            ...
            {
                if (defenseStarted && (ActiveWaves() == null || !ActiveWaves().IsRunning) && CountHostiles() == 0)
                {
                    defenseIndex++;
                ...
                if (CountHostiles() >= config.retreatThreshold)
                {
                    ...
                    exitActive = true;
                    if (compass != null && config != null)
                    {
                        compass.SetTarget(config.exitPoint, "Salida");
                    }
                    if (missionUI != null)
                    {
        ...
        private void SpawnDefenseAllies()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                pool.Spawn(Enemies.Faction.Sena, rally + new Vector3(-4f + 8f * i, 0.5f, -4f), rally, "morro_aliado_" + i);
            }
        }
