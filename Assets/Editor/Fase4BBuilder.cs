...
public static class Fase4BBuilder
{
    ...
    public static void BuildAll()
    {
        ...
        {
            for (int i = agentsRoot.transform.childCount - 1; i >= 0; i--)
            {
                Object.DestroyImmediate(agentsRoot.transform.GetChild(i).gameObject);
            }
        }
        int slot = 0;
    ...
    {
        Material debrisMat = MaterialFactory.New();
    ...
    private static GameObject BuildEmitter(string name, Vector3 pos, Color color, float size, float speed, float lifetime)
    {
        ...
        Material fxMat = MaterialFactory.New();
        fxMat.color = color;
