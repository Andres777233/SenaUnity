...
public static class Fase5ABuilder
{
    ...
    private static Vector3[] BuildCoverMarkers(Vector3[] route)
    {
        ...
        {
            GameObject viejo = GameObject.Find("CoverM2_" + i);
            if (viejo != null)
            {
    ...
    private static void Rebake(Vector3 camp, Vector3 fight, Vector3 morro)
    {
        ...
        string[] viejosVol = new string[] { "NavCamp", "NavFight", "NavMorro" };
        for (int i = 0; i < viejosVol.Length; i++)
        {
            GameObject viejo = GameObject.Find(viejosVol[i]);
            if (viejo != null)
            {
                Object.DestroyImmediate(viejo);
            }
